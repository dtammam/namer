...
- [ ] Running `namer` without `--things` produces output that draws nouns from
      the objects list (default behaviour unchanged).
- [ ] Running `namer --things objects` produces the same default behaviour
      (explicit selection equals the default).
- [ ] Running `namer --things produce` produces output that draws nouns from the
      produce list and not from the objects or animals list.
- [ ] Running `namer --things animals` produces output that draws nouns from the
      animals list and not from the objects or produce list.
- [ ] Running `namer --things invalid` exits with a non-zero status code and
      prints a helpful error message (via clap's default error handling).
- [ ] `namer --help` output includes `--things`, lists its accepted values
      (`objects`, `produce`, `animals`), and shows the default (`objects`).
- [ ] The exact-777-entry assertion (`word_lists_contain_exactly_777_entries_each`)
...
- [ ] No word appears in both the adjectives list and any noun category list.
      Verified by a unit test.
- [ ] `--things` works correctly in combination with `--lower`, `--delimiter`,
      and `--number` (integration tests cover at least one combination per
      category).
- [ ] All public functions and types introduced or modified by this feature have
      doc comments.
- [ ] `cargo fmt -- --check` passes.
- [ ] `cargo clippy -- -D warnings` passes (zero warnings).
- [ ] `cargo test` passes (unit + integration).
- [ ] Build time < 10 s; test suite < 10 s.
