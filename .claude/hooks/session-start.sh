...
if [ -f "$STATE_FILE" ] && [ -s "$STATE_FILE" ] && [ "$(cat "$STATE_FILE")" != "{}" ]; then
  FEATURE_NAME="$(grep -o '"feature_name"[[:space:]]*:[[:space:]]*"[^"]*"' "$STATE_FILE" 2>/dev/null | head -1 | sed 's/.*: *"//' | sed 's/"//' || true)"
  FEATURE_STAGE="$(grep -o '"stage"[[:space:]]*:[[:space:]]*"[^"]*"' "$STATE_FILE" 2>/dev/null | head -1 | sed 's/.*: *"//' | sed 's/"//' || true)"
fi
...
if [ -n "$FEATURE_NAME" ] && [ -n "$FEATURE_STAGE" ]; then
